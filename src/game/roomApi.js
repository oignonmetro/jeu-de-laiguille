...
// la somme, ou un malus (cf. noGuessPenalty) si son indice n'a rapporté aucun
// point à personne — puis on passe le tour en "reveal". Transaction pour
// qu'un seul calcul ait lieu même si les dernières réponses arrivent en même
// temps.
export async function submitIndividualGuess(roomCode, turnIndex, playerId, guessedAngle) {
  ...
  await runTransaction(roomRef, (room) => {
    ...
    if (guessers.every((id) => room.guesses[id] != null)) {
      ...
      // Malus : un indice qui ne fait marquer personne coûte des points à
      // l'auteur, proportionnellement au nombre de joueurs.
      if (authorScore === 0) authorScore = noGuessPenalty(room.order.length)
